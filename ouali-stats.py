...
for infile in ouali_established_files:
    ...
    ouali_counts = ouali_data['décision d\'alignement'].value_counts(sort=False)
    counts.update({instance_name:ouali_counts})
...
for infile in ouali_undefined_files:
    instance_name = re.search(r'(\w+-\w+)',infile).group()
    ouali_data = pd.read_csv(infile, sep='\t', dtype = str).size
